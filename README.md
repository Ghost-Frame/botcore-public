...
Cloud APIs (OpenAI, Together, OpenRouter, etc.) also work -- any service with an OpenAI-compatible chat completions endpoint. Just set `llmUrl` to the API endpoint and add your API key to the request headers via the `llmUnixSocket` override or a proxy.
